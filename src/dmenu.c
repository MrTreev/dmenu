...
static void drawmenu(void) {
    ...
    if (lines > 0) {
        ...
        x += w;
        for (item = curr; item != next; item = item->right) { x = drawitem(item, x, 0, MIN(TEXTW(item->text), mw - x - TEXTW(">"))); }
        if (next) {
            w = TEXTW(">");
            drw_setscheme(drw, scheme[SchemeNorm]);
...
static void grabkeyboard(void) {
    ...
    for (i = 0; i < 1000; i++) {
        if (XGrabKeyboard(dpy, DefaultRootWindow(dpy), True, GrabModeAsync, GrabModeAsync, CurrentTime) == GrabSuccess) { return; }
        nanosleep(&ts, NULL);
    }
...
static void match(void) {
    ...
    for (s = strtok(buf, " "); s; tokv[tokc - 1] = s, s = strtok(NULL, " ")) {
        if (++tokc > tokn && !(tokv = realloc(tokv, ++tokn * sizeof *tokv))) { die("cannot realloc %u bytes:", tokn * sizeof *tokv); }
    }
...
static void keypress(XKeyEvent *ev) {
    ...
    switch (status) {
        default: /* XLookupNone, XBufferOverflow */ return;
        case XLookupChars: goto insert;
        case XLookupKeySym:
    ...
    if (ev->state & ControlMask) {
        switch (ksym) {
            ...
            case XK_k: /* delete right */
                ...
                match();
                break;
            case XK_u: /* delete left */ insert(NULL, 0 - cursor); break;
            case XK_w: /* delete word */
                while (cursor > 0 && strchr(worddelimiters, text[nextrune(-1)])) { insert(NULL, nextrune(-1) - cursor); }
                while (cursor > 0 && !strchr(worddelimiters, text[nextrune(-1)])) { insert(NULL, nextrune(-1) - cursor); }
                break;
            case XK_y: /* paste selection */
            case XK_Y: XConvertSelection(dpy, (ev->state & ShiftMask) ? clip : XA_PRIMARY, utf8, utf8, win, CurrentTime); return;
            case XK_Left: movewordedge(-1); goto draw;
            case XK_Right: movewordedge(+1); goto draw;
            case XK_Return: /* fallthrough */
            case XK_KP_Enter: break;
            case XK_bracketleft: cleanup(); exit(1);
            default: return;
        }
    } else if (ev->state & Mod1Mask) {
        switch (ksym) {
            case XK_b: movewordedge(-1); goto draw;
            case XK_f: movewordedge(+1); goto draw;
...
static void buttonpress(XEvent *e) {
    ...
    if (ev->state & ~ControlMask) { return; }
    if (lines > 0) {
...
static void readstdin(void) {
    ...
    for (i = 0; fgets(buf, sizeof buf, stdin); i++) {
        if (i + 1 >= size / sizeof *items) {
            if (!(items = realloc(items, (size += BUFSIZ)))) { die("cannot realloc %u bytes:", size); }
        }
        if ((p = strchr(buf, '\n'))) { *p = '\0'; }
        if (!(items[i].text = strdup(buf))) { die("cannot strdup %u bytes:", strlen(buf) + 1); }
        items[i].out = 0;
...
static void setup(void) {
...
#endif
    /* init appearance */
    for (j = 0; j < SchemeLast; j++) { scheme[j] = drw_scm_create(drw, colors[j], alphas[j], 2); }
    ...
    win                   = XCreateWindow(dpy, parentwin, x, y, mw, mh, 0, depth, InputOutput, visual, CWOverrideRedirect | CWBackPixel | CWColormap | CWEventMask | CWBorderPixel, &swa);
    ...
    if (embed) {
        ...
        if (XQueryTree(dpy, parentwin, &dw, &w, &dws, &du) && dws) {
            for (i = 0; i < du && dws[i] != win; ++i) { XSelectInput(dpy, dws[i], FocusChangeMask); }
            XFree(dws);
        }
        grabfocus();
    }
