...
void drw_clr_create(Drw *drw, Clr *dest, const char *clrname, unsigned int alpha) {
    ...
    if (!XftColorAllocName(drw->dpy, drw->visual, drw->cmap, clrname, dest)) { die("error, cannot allocate color '%s'", clrname); }
...
Clr *drw_scm_create(Drw *drw, const char *clrnames[], const unsigned int alphas[], size_t clrcount) {
    ...
    for (i = 0; i < clrcount; i++) { drw_clr_create(drw, &ret[i], clrnames[i], alphas[i]); }
    return ret;
}
